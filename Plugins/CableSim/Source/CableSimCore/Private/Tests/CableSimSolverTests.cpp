...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FCableSimSlideOverEdgeTest,
	"CableSim.Core.Manifold.SlideOverEdge",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FCableSimSlideOverEdgeTest::RunTest(const FString& Parameters)
{
	auto MakeTri = [](const int32 Index, const int32 I0, const int32 I1, const int32 I2,
		const FVector3d& A, const FVector3d& B, const FVector3d& C)
	{
	...
	TArray<CableSim::FCollisionTriangle> Triangles;
	Triangles.Add(MakeTri(0, 0, 1, 2, FVector3d(0, -60, 0), FVector3d(0, 60, 0), FVector3d(-60, 0, -60)));
	Triangles.Add(MakeTri(1, 1, 0, 3, FVector3d(0, 60, 0), FVector3d(0, -60, 0), FVector3d(60, 0, -60)));
	TArray<CableSim::FCollisionEdge> Edges;
	TArray<CableSim::FCollisionVertex> Vertices;
	CableSim::FCollisionTopologyCompiler::CompileTopology(Triangles, 0.1, 8, Edges, Vertices);

	const double SlideSpeed = 30.0;
	auto MeasureSlide = [&](const double OverRelax, const int32 Iterations, int32& OutFirstSpikeStep)
	{
		CableSim::FSimulationConfig Config = CableSimTests::MakeSettleConfig(200.0, 10.0);
		Config.DistanceOverRelaxation = OverRelax;
		Config.ConstraintIterations = Iterations;
		CableSim::FSolver Solver;
		Solver.Initialize(FVector3d(-80, 0, -30), FVector3d(80, 0, -30), Config);
		CableSim::FManifoldConfig ManifoldConfig;
		const CableSim::FContactGenerator Generator =
			[&Triangles, &Edges, ManifoldConfig](const TConstArrayView<CableSim::FParticle> Particles,
				TArray<CableSim::FContactConstraint>& Contacts)
		...
		FVector3d StartTarget(-80, 0, -30);
		FVector3d EndTarget(80, 0, -30);
		for (int32 Step = 0; Step < 200; ++Step)
		{
			Solver.AdvanceStep(CableSimTests::MakeFixedInput(StartTarget, EndTarget), Generator);
		}
		const double Delta = SlideSpeed / 60.0;
		double MaxNodeSpeed = 0.0;
		OutFirstSpikeStep = -1;
		for (int32 Step = 0; Step < 300; ++Step)
		{
			StartTarget.X += Delta;
			EndTarget.X += Delta;
			CableSim::FStepInput Input = CableSimTests::MakeFixedInput(StartTarget, EndTarget);
			Input.StartEndpoint.TargetVelocity = FVector3d(SlideSpeed, 0, 0);
			Input.EndEndpoint.TargetVelocity = FVector3d(SlideSpeed, 0, 0);
			const CableSim::FStepResult StepResult = Solver.AdvanceStep(Input, Generator);
			const double Speed = StepResult.MaximumParticleSpeed;
			if (OutFirstSpikeStep < 0 && Speed > SlideSpeed * 4.0)
			{
				OutFirstSpikeStep = Step;
				const TArray<CableSim::FParticle>& P = Solver.GetParticles();
				int32 FastIndex = 0;
				for (int32 I = 1; I < P.Num(); ++I)
				{
					if (P[I].Velocity.SquaredLength() > P[FastIndex].Velocity.SquaredLength())
					{
						FastIndex = I;
					}
				}
				AddInfo(FString::Printf(
					TEXT("Spike@%d node %d: pos(%.1f,%.1f,%.1f) prev(%.1f,%.1f,%.1f) contacts=%d maxPen=%.2f"),
					Step, FastIndex,
					P[FastIndex].Position.X, P[FastIndex].Position.Y, P[FastIndex].Position.Z,
					P[FastIndex].PreviousPosition.X, P[FastIndex].PreviousPosition.Y, P[FastIndex].PreviousPosition.Z,
					StepResult.ContactCount, StepResult.MaximumPenetration));
			}
			MaxNodeSpeed = FMath::Max(MaxNodeSpeed, Speed);
		}
		return MaxNodeSpeed;
	};

	int32 SpikeA = 0;
	const double MaxSpeed = MeasureSlide(1.015, 64, SpikeA);
	AddInfo(FString::Printf(
		TEXT("Slide over edge at %.0f cm/s: max node speed %.0f cm/s (spike@%d) [KNOWN: drag-over-contact instability]"),
		SlideSpeed, MaxSpeed, SpikeA));
	TestTrue(TEXT("Slide does not numerically fail"), FMath::IsFinite(MaxSpeed));
	return true;
}
